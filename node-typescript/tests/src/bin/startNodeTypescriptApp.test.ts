...
function getFileContent(paths: string[]): string {
    return fs.readFileSync(resolve(...paths), {
        encoding: "utf-8"
    });
}
...
describe("Feature: cli", () => {
    ...
    before(() => {
        ...
        spawnStub
            .withArgs("npm", match.array.contains(["i", "-D"]), match.any)
            .returns({
                    stdout: new Stream(),
                    stderr: new Stream(),
    ...
    context("given the app directory does not exist", () => {
        context(`when startNodeTypescriptApp("app") is called`, () => {
            ...
            before(async function() {
                ...
                packageJson = require(resolve(appDirectory, "package.json")) as Record <
                    string,
                    unknown >
                ;
            });
            ...
            it("should create the src/index.ts file", () => {
                expect(getFileContent([appDirectory, "src", "index.ts"])).to.equal(
            ...
            it("should create the tests/src/index.test.ts file", () => {
                ...
                ).to.equal(getFileContent([root, "tests", "src", "index.test.ts"]));
            });

            it("should create the tests/setup.ts file", () => {
                expect(getFileContent([appDirectory, "tests", "setup.ts"])).to.equal(
                    getFileContent([root, "tests", "setup.ts"])
                );
            });

            it("should create the .env file", () => {
                expect(getFileContent([appDirectory, ".env"])).to.equal(
                    getFileContent([root, ".env.example"])
                );
            });

            it("should create the .eslintignore file", () => {
                expect(getFileContent([appDirectory, ".eslintignore"])).to.equal(
                    getFileContent([root, ".eslintignore"])
                );
            });

            it("should create the .eslintrc.json file", () => {
                const eslintrcTemplate = JSON.parse(
                        getFileContent([root, ".eslintrc.json"])
                    ) as Record < string,
                    unknown > ;
    ...
    context("given the app directory contains an existing project", () => {
        context(`when startNodeTypescriptApp("app") is called`, () => {
            ...
            it("should rename existing files", () => {
                expect(fs.existsSync(resolve(appDirectory, ".env.original"))).to.be
                    .true;

                expect(fs.existsSync(resolve(appDirectory, ".eslintignore.original")))
                    .to.be.true;

                expect(fs.existsSync(resolve(appDirectory, ".eslintrc.json.original")))
                    .to.be.true;

                expect(fs.existsSync(resolve(appDirectory, ".gitignore.original"))).to
                    .be.true;

                expect(fs.existsSync(resolve(appDirectory, ".mocharc.json.original")))
                    .to.be.true;

                expect(fs.existsSync(resolve(appDirectory, ".nycrc.original"))).to.be
                    .true;

                expect(fs.existsSync(resolve(appDirectory, "nodemon.json.original"))).to
                    .be.true;

                expect(fs.existsSync(resolve(appDirectory, "README.md.original"))).to.be
                    .true;

                expect(fs.existsSync(resolve(appDirectory, "tsconfig.json.original")))
                    .to.be.true;
            });
